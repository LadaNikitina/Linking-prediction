...
def preprocessing(x, y, batch_size, top_k, embs, num_clusters, shuffle):
    ...
    x_ = np.concatenate((x[indexes], np.full((batch_size - data_len % batch_size, top_k), null_cluster + 1)), axis = 0)
    y_ = np.concatenate((y[indexes], np.full((batch_size - data_len % batch_size), -1)), axis = 0)
...
#         print(ind_batch, '/', num_batches)
        ...
        # edges[0] - from user in sys
        # edges[1] - from sys in user
        # edges[2] - from null
