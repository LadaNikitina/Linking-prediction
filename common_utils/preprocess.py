...
class Clusters:
    ...
    def get_embeddings(self):
        ...
        '''
        convert_one_prev_embeddings = np.array(pd.read_csv(self.path, index_col=0))

        self.embs_dim = convert_one_prev_embeddings.shape[1]
        # train user/system embeddings
        self.train_user_embs = convert_one_prev_embeddings[self.user_train_index]
        self.train_system_embs = convert_one_prev_embeddings[self.system_train_index]

        # test user/system embeddings
        self.test_user_embs = convert_one_prev_embeddings[self.user_test_index]
        self.test_system_embs = convert_one_prev_embeddings[self.system_test_index]

        # validation user/system embeddings
        self.valid_user_embs = convert_one_prev_embeddings[self.user_valid_index]
        self.valid_system_embs = convert_one_prev_embeddings[self.system_valid_index]
    ...
    def one_stage_clustering(self):
        ...
        '''
        print(f"Intent metric conveRT-one-prev, {self.first_num_clusters} one-stage clusters, \
    ...
    def two_stage_clustering(self):
        ...
        '''
        print(f"Intent metric conveRT-one-prev, {self.second_num_clusters} two-stage clusters, \
...
def get_all_accuracy_printer(k, test_user_data, test_system_data, w_sys_user, w_user_sys, data):
    '''
    ...
    metric = []
    wrong_results = []
    user_predictions = np.array((second_num_clusters, 2))
    system_predictions = np.array((second_num_clusters, 2))
    neg_number = 0
    all_number = 0
    ...
    for obj in data:
        ...
        for i in range(len(obj["utterance"])):
            if obj['speaker'][i] == 0:
                all_number += 1
                cur_cluster = test_user_data["cluster"][ind_user] #user cur
                
                top = []
                    
                for kk in range(len(w_sys_user[ind_user][:])):
                    top.append((w_sys_user[ind_user][kk], kk))
                        
                top.sort(reverse=True)
                top = top[:k]
#                 print(top)

                if (w_sys_user[ind_user][cur_cluster], cur_cluster) in top:
                    utterence_metric.append(1)
                else:
                    utterence_metric.append(0)
                    neg_number += 1
                    
                    copy_ind_user = ind_user - 1
                    copy_ind_system = ind_system - 1

                    clusters = []
                    for kk in range(i - 1, max(i - top_k, 0) - 1, -1):
                        if obj['speaker'][kk] == 0:
                            clusters.append(test_user_data["cluster"][copy_ind_user])
                            copy_ind_user -= 1
                        else:
                            clusters.append(test_system_data["cluster"][copy_ind_system] + second_num_clusters)
                            copy_ind_system -= 1
                    clusters = clusters[::-1]
                    for kk in range(max(i - top_k, 0), i):
                        print(obj["utterance"][kk], clusters[kk - max(i - top_k, 0)])

                    print("----------")        
                    print(obj["utterance"][i], cur_cluster)
                    print(top)
                    print("**********") 
#                     print(obj)
                ...
                for kk in range(len(w_user_sys[ind_system][:])):
                    top.append((w_user_sys[ind_system][kk], kk))
                        
                top.sort(reverse=True)
                top = top[:k]
#                 print(top)

                if (w_user_sys[ind_system][cur_cluster],cur_cluster) in top:
                    utterence_metric.append(1)
                else:
                    utterence_metric.append(0)
                    neg_number += 1
